...
func RegisterAdminRoutes(r chi.Router) {
	...
	r.Get("/admin/servers", middleware.RequireAdmin(h.ListAdminServers))
	r.Get("/admin/servers/{id}", middleware.RequireAdmin(h.GetAdminServer))
	r.Delete("/admin/servers/{id}", middleware.RequireAdmin(h.DeleteAdminServer))
	r.Post("/admin/servers/{id}/suspend", middleware.RequireAdmin(h.SuspendServer))
	r.Post("/admin/servers/{id}/unsuspend", middleware.RequireAdmin(h.UnsuspendServer))
	r.Get("/admin/users", middleware.RequireAdmin(h.ListUsers))
	r.Get("/admin/users/{id}", middleware.RequireAdmin(h.GetUser))
	r.Patch("/admin/users/{id}", middleware.RequireAdmin(h.UpdateUser))
	r.Delete("/admin/users/{id}", middleware.RequireAdmin(h.DeleteUser))
	r.Post("/admin/users/{id}/restrict", middleware.RequireAdmin(h.RestrictUser))
	r.Post("/admin/users/{id}/unrestrict", middleware.RequireAdmin(h.UnrestrictUser))
	r.Post("/admin/users/{id}/unsuspend-all", middleware.RequireAdmin(h.UnsuspendAllUserServers))
	r.Get("/admin/activity", middleware.RequireAdmin(h.AdminActivity))
	r.Get("/admin/nests", middleware.RequireAdmin(h.ListAdminNests))
	r.Post("/admin/nests", middleware.RequireAdmin(h.SaveNest))
	r.Put("/admin/nests/{id}", middleware.RequireAdmin(h.UpdateNest))
	r.Post("/admin/eggs", middleware.RequireAdmin(h.SaveEgg))
	r.Put("/admin/eggs/{nestId}/{eggId}", middleware.RequireAdmin(h.UpdateEgg))
	r.Get("/admin/health", middleware.RequireAdmin(h.HealthCheck))
	r.Get("/admin/stats", middleware.RequireAdmin(h.AdminStats))
	r.Post("/admin/users/{id}/reset-password", middleware.RequireAdmin(h.AdminResetPassword))
	r.Post("/admin/servers/{id}/reinstall", middleware.RequireAdmin(h.AdminReinstallServer))
	r.Get("/admin/logs", middleware.RequireAdmin(h.GetLogs))
	r.Get("/admin/settings", middleware.RequireAdmin(h.GetSettings))
	r.Put("/admin/settings", middleware.RequireAdmin(h.UpdateSettings))
}
